...
- 操作页签，点击 ` 新建 ` 按钮；
  *  ` 操作 ` 选择 ` 启动程序 ` ；
  * 设置 ` 程序或脚本 ` 为应用全路径，例如： ` C:\dns.resolver\dns.exe ` ；
  *  ` 起始于 ` 设置为应用所在文件夹，例如： ` C:\dns.resolver ` 。
- 条件页签，
  * 根据需要选择去除 ` 只有在计算机使用交流电源时才启动此任务 ` 选项。
- 设置页签，
  * 根据需要选择去除 ` 如果任务运行时间超过以下时间，停止任务 ` 选项。
- 点击 ` 确定 ` 按钮，输入用户名和密码，保存任务。
