...
<p align="center"><img src="img/topology.png" /></p>
...
在 windows、Linux 和 macOS 中赋予运行权限，直接运行或打开命令行运行 cnr.exe 或 cnr 应用程序。
...
- **server**
  ...
  * 每个元素采用` “限定域名 + 协议 + IP 地址 + 端口号” `的形式。
    ...
    + 当前支持 udp 和 https 协议；可根据个人需要进行配置。
