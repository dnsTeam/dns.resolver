...
- 触发器页签，点击 ` 新建 ` 按钮；` 开始任务 ` 选择 ` 启动时 `；<br />根据需要设置 ` 延迟任务时间 `，实现启动一段时间后运行。
...
- **bind：本地绑定信息。**
  ...
  * 用作本地 DNS 服务器，设为 ` [ "127.0.0.1" ] ` ( IPv4 ) 或 ` [ "::1" ] ` ( IPv6 ) 。
  * 用作局域网 DNS 服务器，设为 ` [ "A.B.C.D" ] ` ( IPv4 ) 或 ` [ "A:B:C:D:E:F:G:H" ] ` ( IPv6 ) 。
